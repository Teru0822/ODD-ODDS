...
namespace QubicNS
{
    public class Rnd
    {
        ...
        public Rnd(int seed1, int seed2) : this(CombineHashCodes(seed1, seed2))
        {
        }
        ...
        public T GetRndItem<T>(IEnumerable<T> items, Func<T, float> chance)
        {
            ...
            {
                var ch = chance(item);
                totalWeight += ch; // Accumulate the sum of chances

                // Replace the current selection with probability (Chance / totalWeight)
                if (Float(0, totalWeight) <= ch)
                    selectedItem = item;
            }
        ...
        public T GetRndItem<T>(IEnumerable<(T item, float chance)> items)
        {
            // Reservoir Sampling method
            var totalWeight = 0f;
            T selectedItem = default;
            ...
            foreach (var item in items)
            {
                totalWeight += item.chance; // Accumulate the sum of chances

                // Replace the current selection with probability (Chance / totalWeight)
                if (Float(0, totalWeight) <= item.chance)
        ...
        public T GetRndItem<T>(IEnumerable<T> items)
        {
            // Reservoir Sampling method
            var totalWeight = 0f;
            T selectedItem = default;
            ...
            foreach (var item in items)
            {
                totalWeight += 1f; // Accumulate the sum of chances

                // Replace the current selection with probability (Chance / totalWeight)
                if (Float(0, totalWeight) <= 1f)
                    selectedItem = item;
            }
        ...
        [Obsolete]
        public T GetRndWithProb<T>(IEnumerable<(T item, float probability)> items)
        {
            if (items == null)
                return default;

            // calc total probability
            float totalWeight = items.Sum(i => i.probability);
            if (totalWeight <= 0)
                return default;

            // generate random value from 0 to totalWeight
            float randomValue = (float)(Float() * totalWeight);
        ...
        /// <summary>
        /// Returns index according to probabilities
        /// </summary>
        ...
        /// <summary>
        /// Returns value according to probabilities
        /// </summary>
        ...
        public static float Spatial(int x, int y)
        {
        ...
        /// <summary>Spatial random from 0 to 1</summary>
        public static float Spatial(int x, int y, int z)
        {
            var p3_x = Fract(x * 0.1031f);
            var p3_y = Fract(y * 0.1031f);
        ...
        public static int SpatialInt(int x, int y)
        {
        ...
        /// <summary>Spatial random from 0 to int.MaxValue</summary>
        public static int SpatialInt(int x, int y, int z)
        {
            var p3_x = Fract(x * 0.1031d);
            var p3_y = Fract(y * 0.1031d);
        ...
        public Rnd GetBranch(int seed1, int seed2, int seed3)
        {
            return new Rnd(CombineHashCodes(stackalloc[] { this.seed, seed1, seed2, seed3 }));
        }
        ...
        /// <summary>
        /// Stable hashcode for list of value types. Does not allocate memory!
        /// </summary>
        /// <example>
        /// int hash = GetStableHashCode(stackalloc[] { 1, 2, 3, 4 });
        /// </example>
        public static int CombineHashCodes<T>(ReadOnlySpan<T> values) where T : struct
        {
            unchecked
            {
                int hash = 17;
        ...
        /// int hash = GetStableHashCode(stackalloc[] { 1, 2, 3, 4 });
        /// </example>
        public static int CombineHashCodes(ReadOnlySpan<int> values)
        {
            unchecked
            {
                int hash = 17;
